...
function* databaseRead({payload, metadata}) {
  /*--- Validate Request ---*/
  /*--- Try Request ---*/
  try {
    /*--- Destructure ---*/
...
function* databaseWrite({payload, metadata}) {
  ...
  try {
    ...
    const documentInserted = yield call(hashDOM, payload, metadata)
    switch(writeType) {
      case 'create':
        request = yield call(reduxSagaFirebase.database.create, `/${entity}/${branch.join("/")}`, {hashes:documentInserted});
        break;
      case 'update':
        request = yield call(reduxSagaFirebase.database.update, `/${entity}/${branch.join("/")}`, {hashes:documentInserted});
        break;
      case 'patch':
        request = yield call(reduxSagaFirebase.database.patch, `/${entity}/${branch.join("/")}`, {hashes:documentInserted});
        break;
    default:
...
async function hashDOM(payload, metadata) {
